...
class RayCastedMap(Map):
    ...
    def do_fov(self, px: int , py: int, radius: int):
        ...
        self.reset_light()
        self.set_lit(px, py)
        for i in range(0, RayCastedMap.RAYS, RayCastedMap.STEP):
            ax = sintable[i]
            ay = costable[i]
            ...
            for z in range(radius):
                ...
                if not (0 <= x < self.width and 0 <= y < self.height):
                    break
                self.set_lit(int(round(x)), int(round(y)))
                if self.is_blocked(int(round(x)), int(round(y))):
                    break

if __name__ == "__main__":
    print('Usage: py -m maps -m ray')
