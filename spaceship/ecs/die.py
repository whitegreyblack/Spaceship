...
def check_sign(value, save_zero=False):
    """
    Converts all int values into string representations. If value is positive, 
    a plus sign is added to the string. If value is zero, then function 
    returns an empty string when save_zero is false else it returns the zero 
    as a string.
    """
    string = ""
...
class Die:
    """
    Die class used in determining a controlled random number. Can be used with 
    any attribute, chance mechanic or random event occurance.
    ...
    def __init__(self, sides=6, multiplier=1, modifier=0):
        """
        Checks all three parameters for correct values. If incorrect values 
        are found then init raises ValueErrors and class creation is aborted.
        """
        if sides < 0:
    ...
    def eval_dice_string(string):
        """
        Used to recognize valid dice strings. Matches input string with regex 
        used in determining dice strings. If match found then converts die 
        string into three integer parameters used in initializing dices.
        """
    ...
    def construct(cls, string):
        """Creates a Die object from string if string is a valid die string"""
